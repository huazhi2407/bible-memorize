...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    const plan = await getScripturePlan(year, week);
    if (!plan) return res.json(null);
    res.json({ id: plan.id, year: plan.year, week: plan.week, segments: plan.segments });
  } catch (e) {
    console.error(e);
...
router.post('/', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const id = await createOrUpdateScripturePlan(Number(year), Number(week), segments);
